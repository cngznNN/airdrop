...
#define MAX_DROPS (10)
#define COLOR_DROP (0) // Deðeri 0'dan farklý yaparsanýz girdiðiniz obje modelinde drop düþer. Örneðin: 19332.
...
static const Float:g_arrDropSpawn[][e_dropSpawn] = {
	{340.6458,-1486.5393,76.5391} // Random spawn olucak airdrop'larýn X, Y, Z koordinatlarýný girin.
};
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	if(!strcmp(cmdtext, "/createdrop", true))
	{
		for(new i = 0; i < MAX_DROPS; i++) if(!DropInfo[i][dropExists])
		{
				...
				MoveObject(DropInfo[i][dropObject], DropInfo[i][dropPos][0], DropInfo[i][dropPos][1], DropInfo[i][dropPos][2]-1, 10.0);
				SendClientMessage(playerid, -1, "Ovyeah");
			#else
	...
	else if(!strcmp(cmdtext, "/drop", true))
	{
		...
		if((id = Nearest_AirDrop(playerid)) != -1)
		{
			...
			new string[128];
			format(string, sizeof(string), "Silahlar\n%s\n%s\n%s", (!DropInfo[id][dropHealth]) ? ("Boþ Slot") : ("Saðlýk"), (!DropInfo[id][dropArmor]) ? ("Boþ Slot") : ("Çelik Yelek"),
			(DropInfo[id][dropSkin] == -1) ? ("Boþ Slot") : ("Skin"));
			ShowPlayerDialog(playerid, 0, DIALOG_STYLE_LIST, "Airdrop", string, "Seç", "Ýptal");
			SetPVarInt(playerid, "DropID", id);
		}
		else SendClientMessage(playerid, -1, "Airdrop yakýnlarýnda deðilsiniz.");
